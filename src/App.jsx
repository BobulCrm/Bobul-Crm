...
const T = {
bg:”#F7F3EE”, surface:”#FFFFFF”, card:”#FBF8F4”, cardHover:”#F2EDE5”,
border:”#E4DDD3”, borderGold:”#B8922040”, text:”#2C2420”, textMuted:”#8A7968”,
textDim:”#C4B8AC”, gold:”#9A6F20”, goldLight:”#C49A2A”, goldDim:”#9A6F2015”,
accent:”#9A6F20”, grad:“linear-gradient(135deg,#B8922A 0%,#7A5410 100%)”,
green:”#3D7A52”, red:”#B04535”, blue:”#3A6A9A”,
};

const css = `@import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,500;0,600;0,700;1,400;1,500&family=Jost:wght@300;400;500;600&display=swap'); *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;} html,body{background:${T.bg};} ::-webkit-scrollbar{width:3px;height:3px;} ::-webkit-scrollbar-thumb{background:${T.border};border-radius:2px;} @keyframes fadeUp{from{opacity:0;transform:translateY(16px);}to{opacity:1;transform:translateY(0);}} @keyframes fadeIn{from{opacity:0;}to{opacity:1;}} @keyframes slideR{from{opacity:0;transform:translateX(24px);}to{opacity:1;transform:translateX(0);}} @keyframes glow{0%,100%{box-shadow:0 0 0px #9A6F2000;}50%{box-shadow:0 0 20px #9A6F2033;}} .fadeUp{animation:fadeUp .45s cubic-bezier(.22,.68,0,1.2) both;} .fadeIn{animation:fadeIn .3s ease both;} .pBtn{transition:all .2s;cursor:pointer;border:none;} .pBtn:hover{transform:translateY(-1px);filter:brightness(1.1);} .pBtn:active{transform:translateY(0);filter:brightness(.95);} .profCard{transition:all .2s;cursor:pointer;border:1px solid ${T.border};} .profCard:hover{border-color:${T.gold}80!important;background:${T.cardHover}!important;transform:translateY(-1px);} .clientCard{transition:all .2s;cursor:pointer;} .clientCard:hover{background:${T.cardHover}!important;border-color:${T.gold}50!important;transform:translateY(-1px);box-shadow:0 4px 16px rgba(44,36,32,.08)!important;} .rowHover{transition:background .15s;cursor:pointer;} .rowHover:hover{background:${T.cardHover}!important;} .tabBtn{transition:all .2s;cursor:pointer;border:none;} .tabBtn:hover{color:${T.gold}!important;} input,textarea,select{outline:none;transition:border-color .2s;} input:focus,textarea:focus,select:focus{border-color:${T.gold}!important;box-shadow:0 0 0 2px ${T.goldDim};} select option{background:#FFFFFF;color:#2C2420;} .goldLine{height:1px;background:linear-gradient(90deg,transparent,${T.gold}50,transparent);} .tag{display:inline-flex;align-items:center;gap:4px;padding:2px 8px;border-radius:1px;font-size:10px;letter-spacing:.5px;font-family:'Jost',sans-serif;}`;
...
// —  —CRM MAIN —————————————————————–
function CRM({config,user,supabase,plan,onUpgrade,onReset,onSignOut}){
...
notes:””,expectedClose:today(),currency:“USD”,
...
reader.onload = (ev) => {
fileObj.dataUrl = ev.target.result;
const updated = {…selected, files: […(selected.files||[]), fileObj]};
...
const updated = {…selected, files:(selected.files||[]).filter(f=>f.id!==fileId)};
setClients(clients.map(c=>c.id===selected.id?updated:c));
setSelected(updated);
...
```
{/* -- TOPBAR */}
<div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"0 18px",display:"flex",alignItems:"center",height:54,gap:10,position:"sticky",top:0,zIndex:100}}>
  <div style={{display:"flex",alignItems:"center",gap:9,marginRight:6}}>
    <span style={{fontSize:16,color:T.gold}}>◈</span>
    <div>
      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:15,fontWeight:600,color:T.text,lineHeight:1}}>{company||name}</div>
      <div style={{fontSize:9,color:T.textMuted,letterSpacing:1,textTransform:"uppercase"}}>{profession?.label?.slice(0,30)}</div>
    </div>
  </div>
  ...
  <div style={{flex:1,maxWidth:240,position:"relative"}}>
    ...
    <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar cliente..."
      style={{...inputS,padding:"6px 9px 6px 25px",fontSize:12}}/>
  </div>

  <select value={filterStage} onChange={e=>setFilterStage(e.target.value)} style={{background:T.card,border:`1px solid ${T.border}`,color:filterStage==="all"?T.textMuted:T.gold,borderRadius:2,padding:"6px 10px",fontSize:11,fontFamily:"'Jost',sans-serif"}}>
    <option value="all">Todas las etapas</option>
    {STAGES.map(s=><option key={s}>{s}</option>)}
  </select>

  <div style={{display:"flex",border:`1px solid ${T.border}`,borderRadius:2,overflow:"hidden",marginLeft:"auto"}}>
    {[["kanban","Kanban"],["list","Lista"],["stats","Stats"],["tasks","Tareas"]].map(([v,label])=>(
      <button key={v} className="pBtn" onClick={()=>setView(v)} style={{padding:"6px 12px",fontSize:10,letterSpacing:1,textTransform:"uppercase",background:view===v?T.gold+"20":"transparent",color:view===v?T.gold:T.textMuted,borderRight:v!=="tasks"?`1px solid ${T.border}`:"none"}}>{label}{v==="tasks"&&pendingTasks>0?` (${pendingTasks})`:""}</button>
    ))}
  </div>

  <button className="pBtn" onClick={exportCSV} title="Exportar CSV" style={{background:"transparent",color:T.textMuted,padding:"6px 10px",borderRadius:2,fontSize:12,border:`1px solid ${T.border}`}}>↓ CSV</button>
  <button className="pBtn" onClick={()=>{setShowForm(true);setEditing(null);setForm(blankForm());}} style={{background:T.grad,color:"#0C0A08",padding:"7px 16px",borderRadius:2,fontSize:10,fontWeight:700,letterSpacing:2,textTransform:"uppercase"}}>+ Nuevo</button>
  <button className="pBtn" onClick={onReset} title="Configuración" style={{background:"transparent",color:T.textMuted,padding:"6px 9px",borderRadius:2,fontSize:13,border:`1px solid ${T.border}`}}>⚙</button>
  {installPromptCRM&&!installedCRM&&(
    <button className="pBtn" onClick={handleInstallCRM} style={{background:T.grad,color:"#0C0A08",padding:"6px 11px",borderRadius:2,fontSize:10,fontWeight:700,letterSpacing:1,textTransform:"uppercase"}}>⬇ Instalar app</button>
  )}
  <button className="pBtn" onClick={onSignOut} title="Cerrar sesión" style={{background:"transparent",color:T.red,padding:"6px 9px",borderRadius:2,fontSize:11,border:`1px solid ${T.red}30`,letterSpacing:.5}}>Salir</button>
</div>
...
<div style={{flex:1,overflow:"auto",padding:18}}>
  ...
  {/* KANBAN */}
  {view==="kanban"&&(<div style={{display:"flex",gap:9,minHeight:"calc(100vh-200px)",overflowX:"auto",paddingBottom:14}}>
    {STAGES.map(stage=>{
      const scs=filtered.filter(c=>c.stage===stage);
      const col=sc(stage);
      const total=scs.reduce((s,c)=>s+Number(c.value||0),0);
      return(<div key={stage} onDragOver={e=>{e.preventDefault();setDragOver(stage);}} onDrop={()=>{if(dragging){moveStage(dragging,stage);setDragging(null);setDragOver(null);}}} onDragLeave={()=>setDragOver(null)} style={{flex:"0 0 196px",background:dragOver===stage?T.cardHover:T.surface,borderRadius:2,border:`1px solid ${dragOver===stage?col+"80":T.border}`,display:"flex",flexDirection:"column",transition:"all .2s"}}>
                ...
                {catFiles.map(file=>(
                  <div key={file.id} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 10px",background:T.card,border:`1px solid ${T.border}`,borderRadius:2,marginBottom:5}}>
                    ...
                    <div style={{display:"flex",gap:5,flexShrink:0}}>
                      ...
                      {file.dataUrl&&file.type.includes("image")&&(
                        <button className="pBtn" onClick={()=>window.open(file.dataUrl,"_blank")} title="Ver"
                      ...
                      {file.dataUrl&&file.type.includes("pdf")&&(
                        <button className="pBtn" onClick={()=>window.open(file.dataUrl,"_blank")} title="Ver PDF"
                          style={{background:"transparent",border:`1px solid ${T.border}`,color:T.gold,padding:"4px 8px",borderRadius:2,fontSize:11}}>👁</button>
                      )}
...
{/* -- FORM MODAL */}
{showForm&&(<div style={{position:"fixed",inset:0,background:"rgba(44,36,32,.6)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",animation:"fadeIn .2s",padding:16}} onClick={()=>setShowForm(false)}>
  <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:2,width:540,maxHeight:"92vh",display:"flex",flexDirection:"column",animation:"fadeUp .25s"}} onClick={e=>e.stopPropagation()}>
    ...
    <div style={{flex:1,overflowY:"auto",padding:"18px 24px"}}>
      ...
      <div style={{marginBottom:14,borderBottom:`1px solid ${T.border}`,paddingBottom:14}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          ...
          <div><Label>Moneda</Label><select value={form.currency} onChange={e=>setForm({...form,currency:e.target.value})} style={inputS}>{["USD","MXN","EUR","COP","ARS","CLP","PEN"].map(c=><option key={c}>{c}</option>)}</select></div>
          ...
          <div><Label>Cierre esperado</Label><input type="date" value={form.expectedClose} onChange={e=>setForm({...form,expectedClose:e.target.value})} style={inputS}/></div>
          <div><Label>Último contacto</Label><input type="date" value={form.lastContact||today()} onChange={e=>setForm({...form,lastContact:e.target.value})} style={inputS}/></div>
        </div>
      </div>
...
if(!plan.priceId){onSelectPlan(“free”);return;}
setLoading(planKey);
try{
// Load Stripe
...
const stripeObj=window.Stripe(STRIPE_PK);
const{error}=await stripeObj.redirectToCheckout({
lineItems:[{price:plan.priceId,quantity:1}],
mode:“subscription”,
successUrl:`${window.location.origin}?session_id={CHECKOUT_SESSION_ID}&plan=${planKey}`,
...
console.error(e);
// Fallback: just set plan for now
...
// Full CRM
return(<><style>{css}</style><CRM config={config} user={user} supabase={sb} plan={plan} onUpgrade={()=>setShowPricing(true)} onReset={()=>setConfig(null)} onSignOut={handleSignOut}/></>);
}