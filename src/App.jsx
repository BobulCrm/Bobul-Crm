...
function CRM({config,user,supabase,plan,subscriptionEnd,themeId,onThemeChange,onUpgrade,onReset,onSignOut}){
  ...
  const saveEventNow = async (toSave) => {
    ...
    if(editingEvent){
      ...
      track("event_created", {type: newEvent.type, has_client: !!newEvent.clientId, duration_min: Number(newEvent.duration)||60});
      if(supabase&&user){
        const res = await trySupabase(
  ...
  const save=async()=>{
    ...
    if(editing){
      ...
      if(supabase){
        ...
        track("client_created", {source:"manual", has_value: Number(form.value)>0, has_email: !!form.email, has_phone: !!form.phone, profession_id: currentProfId});
  ...
  const moveStage=async(id,stage)=>{
    ...
    showN(`→ ${stage}`);
    track("stage_changed", {from: previous?.stage, to: stage, is_won: isWonStage(stage), is_lost: isLostStage(stage)});
    if(supabase){
      await trySupabase(
  ...
  const addTask=async()=>{
    ...
    track("task_created", {priority: taskPriority, has_due_date: !!taskDue});
    if(supabase){
      await trySupabase(
  ...
  const addPayment = async (clientId, paymentData) => {
    ...
    track("payment_recorded", {amount_range: paymentData.amount<1000?"<1k":paymentData.amount<10000?"1k-10k":paymentData.amount<50000?"10k-50k":">50k", status: paymentData.status, has_method: !!paymentData.method, has_invoice: !!paymentData.invoiceNo});
  };
  ...
  const updatePayment = async (paymentId, updates) => {
    ...
    if(previous.status !== updated.status){
      track("payment_status_changed", {from: previous.status, to: updated.status});
    }
  };
    ...
    <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,position:"sticky",top:0,zIndex:100}}>
      ...
      <div style={{display:"flex",borderTop:`1px solid ${T.border}`,overflowX:"auto"}}>
        {[["kanban","Kanban"],["list","Lista"],["stats","Stats"],["tasks",`Tareas${pendingTasks>0?" ("+pendingTasks+")":""}`],["calendar","Agenda"],["recurrent","Recurrentes"],["payments","Pagos"]].map(([v,label])=>(
          <button key={v} className="pBtn" onClick={()=>{setView(v);track("view_changed",{view:v});}} style={{
      ...
      {view==="stats"&&(<div style={{maxWidth:800,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
        ...
        {(()=>{
          ...
          const exportReportExcel = async () => {
            ...
            track("report_exported", {format:"xlsx", period: reportPeriod, prospects: newClients.length, won: wonClients.length});
          };
          ...
          const exportReportPDF = async () => {
            ...
            track("report_exported", {format:"pdf", period: reportPeriod, prospects: newClients.length, won: wonClients.length});
          };
          ...
          <div style={{padding:"22px 24px 18px"}}>
            ...
            <div style={{display:"flex",gap:8}}>
              <button className="pBtn" onClick={()=>{
                ...
                setDetailTab("info");
                track("followup_alert_action", {action:"view_opportunity", stale_count: staleClients.length});
              ...
              <button className="pBtn" onClick={()=>{setShowFollowUpAlert(false);track("followup_alert_action",{action:"dismiss",stale_count:staleClients.length});}} style={{flex:1,background:"transparent",color:T.textMuted,padding:"11px",borderRadius:2,fontSize:11,fontWeight:600,letterSpacing:1.5,textTransform:"uppercase",border:`1px solid ${T.border}`,cursor:"pointer"}}>
                En otro momento
              </button>
            </div>
          </div>
              ...
              <div>
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 14px",background:T.surface,border:`1px solid ${T.border}`,borderRadius:2,marginBottom:14}}>
                  ...
                  <button className="pBtn" onClick={()=>{
                    ...
                    track("automation_toggled", {feature:"followup_reminder", enabled: next, days: followUpDays});
                  }} style={{
      ...
      <div style={{position:"fixed",inset:0,background:"rgba(44,36,32,.6)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:16,animation:"fadeIn .2s"}} onClick={()=>setShowSubscription(false)}>
        ...
        {showCategoryTip&&(()=>{
          ...
          const switchTo = async (newProfId) => {
            ...
            track("workspace_switched", {from: currentProfId, to: newProfId});
...
function ImportClients({onClose, onImport, currentProfId, profession, stages, existingCount}){
  ...
  const doImport = async () => {
    ...
    setImporting(false);
    setStep(4); // results
    track("clients_imported", {total_attempted: valid.length, successful: success, failed: valid.length-success, profession_id: currentProfId});
  };
