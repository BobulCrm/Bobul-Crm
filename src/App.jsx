  ...
  return(<div style={{minHeight:"100vh",background:T.bg,fontFamily:"Jost,sans-serif",color:T.text,display:"flex",flexDirection:"column"}}>
    ...
    <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"8px 6px",display:"flex",alignItems:"stretch"}}>
      {[
        ...
        {label:["Clientes","activos"],value:visibleClients.filter(c=>!isWonStage(c.stage)&&!isLostStage(c.stage)).length,c:T.text},
        {label:["Total","clientes"],value:visibleClients.filter(c=>!isLostStage(c.stage)).length,c:T.textMuted},
    ...
    <div style={{flex:1,overflow:"auto",overflowX:"hidden",padding:18,maxWidth:"100vw"}}>
      ...
      {view==="stats"&&(<div style={{maxWidth:800,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
        ...
        {(()=>{
          ...
          // Total potential: all NON-LOST clients in the period
          const totalPotential = newClients.filter(c => !isLostStage(c.stage)).reduce((s,c)=>s+Number(c.value||0), 0);
          ...
          const sourceBreakdown = LEAD_SOURCES
            .map(src => {
              const allInSource = newClients.filter(c => c.source === src);
              const wonInSource = allInSource.filter(c => isWonStage(c.stage)).length;
              const lostInSource = allInSource.filter(c => isLostStage(c.stage)).length;
              const decidedInSource = wonInSource + lostInSource;
              const convRate = decidedInSource > 0 ? Math.round((wonInSource / decidedInSource) * 100) : null;
          ...
          const exportReportExcel = async () => {
            ...
            // Fuentes de clientes con conversión
            const sourcesRows = [["Fuente","Clientes","% del período","Ganados","Conversión"]];
            ...
            window.XLSX.utils.book_append_sheet(wb, window.XLSX.utils.aoa_to_sheet(clientsRows), "Clientes nuevos");
            // Eventos
          ...
          const exportReportPDF = async () => {
            ...
            // Metric cards
            const cards = [
              {label:"Clientes nuevos", value:String(newClients.length), color:[201,168,76]},
            ...
            // Detail tables — use simple table layout
            let cursorY = 510;
            ...
            if(sourceBreakdown.length){
              drawTableHeader(`Fuentes de clientes (${sourceBreakdown.length})`);
              drawRow(["FUENTE","CLIENTES","%","GANADOS","CONVERSIÓN"], [200,70,60,70,80], true);
              sourceBreakdown.forEach(({src,count,won,convRate}) => {
                const pct = totalSourced > 0 ? ((count / totalSourced) * 100).toFixed(1) + "%" : "—";
            ...
            if(newClients.length){
              drawTableHeader(`Clientes nuevos (${newClients.length})`);
          ...
          return(
            <div style={{gridColumn:"1/-1",background:T.surface,border:`1px solid ${T.border}`,borderRadius:2,padding:20}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:8}}>
                {[
                  // Clientes (estados)
                  {label:"Clientes nuevos", value:newClients.length, c:T.gold},
              ...
              <div style={{marginTop:18,paddingTop:16,borderTop:`1px solid ${T.border}`}}>
                <div style={{display:"flex",alignItems:"baseline",justifyContent:"space-between",marginBottom:10}}>
                  <div style={{fontSize:9,color:T.gold,letterSpacing:1.5,textTransform:"uppercase",fontWeight:600}}>Fuentes de clientes</div>
                  <div style={{fontSize:9,color:T.textDim,letterSpacing:.5}}>clientes · ganados · conversión</div>
                </div>
                {totalSourced === 0 ? (
                  <div style={{color:T.textDim,fontSize:12}}>Aún no hay clientes con fuente registrada en este período</div>
                ) : (
                  <div style={{display:"flex",flexDirection:"column",gap:8}}>
                    {sourceBreakdown.map(({src,count,won,convRate})=>{
                      ...
                      return(
                        <div key={src} style={{display:"flex",alignItems:"center",gap:10,opacity:isEmpty?.45:1}}>
                          <div style={{flex:"0 0 110px",fontSize:12,color:T.textMuted,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{src}</div>
                          <div style={{flex:1,height:6,background:T.border,borderRadius:1,overflow:"hidden"}}>
                            <div style={{height:"100%",width:`${pct}%`,background:T.gold,transition:"width .4s"}}/>
                          </div>
                          <div style={{flex:"0 0 32px",fontSize:13,fontFamily:"Cormorant Garamond,serif",fontWeight:600,color:isEmpty?T.textDim:T.gold,textAlign:"right"}}>{count}</div>
                          ...
                          <div style={{flex:"0 0 44px",fontSize:11,color:convColor,textAlign:"right",fontWeight:600,letterSpacing:.5}} title={convRate===null?"Sin clientes cerrados todavía":`${won} ganado(s) de los cerrados`}>
