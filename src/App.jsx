...
function CRM({config,user,supabase,plan,subscriptionEnd,themeId,onThemeChange,onUpgrade,onReset,onSignOut}){
  ...
  const exportCSV=()=>{
    ...
    const rows=visibleClients.map(c=>[c.name,c.company||"",c.email||"",c.phone||"",c.value||0,c.stage,c.source||"",(c.tags||[]).join(";"),c.notes||"",c.lastContact||"",fmtDate(c.createdAt)]);
...
export default function App(){
  ...
  // Access gate: must be on Pro plan with non-expired subscription.
  // (Existing users who were promoted to Pro lifetime via SQL pass through.)
  const subActive = subscriptionEnd && new Date(subscriptionEnd) > new Date();
  const hasAccess = plan === "pro" && subActive;
