...
function CRM({config,user,supabase,plan,subscriptionEnd,themeId,onThemeChange,onUpgrade,onReset,onSignOut}){
  ...
  const sc=(stage)=>{
    ...
    if(s.includes("perdid") || s.includes("descart")) return T.red;
    ...
    <div style={{flex:1,overflow:"auto",overflowX:"hidden",padding:18,maxWidth:"100vw"}}>
      ...
      {view==="stats"&&(<div style={{maxWidth:800,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
        ...
        {(()=>{
          ...
          const wonClients = visibleClients.filter(c => c.stage === STAGES[STAGES.length-1] && inRange(c.lastContact || c.createdAt));
          const wonValue = wonClients.reduce((s,c)=>s+Number(c.value||0), 0);
          // Lost opportunities: stage contains "perdid" or "descart" (case/accent-insensitive)
          ...
          // Active pipeline: clients NOT in last stage AND NOT in a lost stage
          // (filtered by period via createdAt for consistency with the rest)
          const activePipelineClients = newClients.filter(c =>
            c.stage !== STAGES[STAGES.length-1] && !isLostStage(c.stage)
          );
          const activePipelineValue = activePipelineClients.reduce((s,c)=>s+Number(c.value||0), 0);
          // Total potential: all clients in the period (any stage)
          const totalPotential = newClients.reduce((s,c)=>s+Number(c.value||0), 0);
          // Lead sources within period
          const sourceBreakdown = LEAD_SOURCES
            .map(src => ({src, count: newClients.filter(c => c.source === src).length}))
            .filter(s => s.count > 0)
          ...
          const exportReportExcel = async () => {
            ...
            // Fuentes de clientes
            const sourcesRows = [["Fuente","Clientes","% del período"]];
            sourceBreakdown.forEach(({src,count}) => {
              const pct = totalSourced > 0 ? ((count / totalSourced) * 100).toFixed(1) + "%" : "—";
              sourcesRows.push([src, count, pct]);
            });
          ...
          const exportReportPDF = async () => {
            ...
            if(sourceBreakdown.length){
              ...
              drawRow(["FUENTE","CLIENTES","%"], [280,80,80], true);
              sourceBreakdown.forEach(({src,count}) => {
                const pct = totalSourced > 0 ? ((count / totalSourced) * 100).toFixed(1) + "%" : "—";
                drawRow([src, String(count), pct], [280,80,80]);
              });
              cursorY += 8;
            }
            ...
            <div style={{gridColumn:"1/-1",background:T.surface,border:`1px solid ${T.border}`,borderRadius:2,padding:20}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:8}}>
                {[
                  ...
                  <div key={i} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:2,padding:"12px 14px",borderLeft:`3px solid ${m.c}`}}>
              ...
              <div style={{marginTop:18,paddingTop:16,borderTop:`1px solid ${T.border}`}}>
                <div style={{fontSize:9,color:T.gold,letterSpacing:1.5,textTransform:"uppercase",fontWeight:600,marginBottom:10}}>Fuentes de clientes</div>
                    ...
                    {sourceBreakdown.map(({src,count})=>{
                      ...
                      return(
                        <div key={src} style={{display:"flex",alignItems:"center",gap:10}}>
                          <div style={{flex:"0 0 100px",fontSize:12,color:T.textMuted,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{src}</div>
                          ...
                          <div style={{flex:"0 0 30px",fontSize:13,fontFamily:"Cormorant Garamond,serif",fontWeight:600,color:T.gold,textAlign:"right"}}>{count}</div>
                        </div>
                      );
                    })}
                  </div>
