          ...
          <div onClick={e=>e.stopPropagation()} style={{
            ...
            <div style={{display:"flex",gap:8}}>
              ...
              <button onClick={handleSend}
                style={{flex:2,background:opened.channel==="whatsapp"?"#25D366":T.grad,color:opened.channel==="whatsapp"?"#FFFFFF":T.gradText,padding:"11px",borderRadius:2,fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase",border:"none",cursor:"pointer"}}>
    ...
    <div>
      ...
      <div style={{display:"flex",gap:8}}>
        ...
        <button onClick={()=>onSave(draft)} disabled={disabled}
          style={{flex:2,background:T.grad,color:T.gradText,padding:"11px",borderRadius:2,fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase",border:"none",cursor:disabled?"not-allowed":"pointer",opacity:disabled?.5:1}}>
          ◈ Guardar
        </button>
      </div>
    </div>
        ...
        {!editing && (
          <>
            <button onClick={()=>setEditing(blank)}
              style={{width:"100%",background:T.grad,color:T.gradText,padding:"10px",borderRadius:2,fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase",border:"none",cursor:"pointer",marginBottom:14}}>
              + Nueva plantilla
            </button>
...
function Onboarding({onFinish}){
  ...
  if(step===0)return(<Center><div className="fadeUp" style={{textAlign:"center",maxWidth:440}}>
    ...
    <button className="pBtn" onClick={()=>setStep(1)} style={{background:T.grad,color:T.gradText,padding:"13px 38px",borderRadius:2,fontSize:12,fontWeight:700,letterSpacing:2,textTransform:"uppercase",boxShadow:`0 4px 20px ${T.gold}40`}}>Comenzar</button>
  ...
  if(step===2)return(<Center><div className="fadeUp" style={{maxWidth:440}}>
    ...
    <div style={{display:"flex",gap:10}}>
      ...
      <button className="pBtn" onClick={()=>setStep(3)} style={{flex:1,background:T.grad,color:T.gradText,padding:"11px",borderRadius:2,fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase"}}>Continuar →</button>
    </div>
  </div></Center>);
  ...
  if(step===3)return(<Center><div className="fadeUp" style={{maxWidth:500,width:"100%"}}>
    ...
    <div style={{display:"flex",gap:10}}>
      ...
      <button className="pBtn" onClick={()=>setStep(4)} style={{flex:1,background:T.grad,color:T.gradText,padding:"11px",borderRadius:2,fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase"}}>Continuar →</button>
    </div>
  </div></Center>);
  ...
  if(step===4)return(<Center><div className="fadeUp" style={{maxWidth:480}}>
    ...
    {showPrivacy&&<div style={{position:"fixed",inset:0,background:"rgba(44,36,32,.6)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={()=>setShowPrivacy(false)}>
      <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:2,padding:28,maxWidth:560,maxHeight:"80vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        ...
        <button className="pBtn" onClick={()=>setShowPrivacy(false)} style={{marginTop:18,background:T.grad,color:T.gradText,padding:"9px 22px",borderRadius:2,fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase"}}>Cerrar</button>
      </div>
    </div>}
    ...
    {showTerms&&<div style={{position:"fixed",inset:0,background:"rgba(44,36,32,.6)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={()=>setShowTerms(false)}>
      <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:2,padding:28,maxWidth:560,maxHeight:"80vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        ...
        <button className="pBtn" onClick={()=>setShowTerms(false)} style={{marginTop:18,background:T.grad,color:T.gradText,padding:"9px 22px",borderRadius:2,fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase"}}>Cerrar</button>
      </div>
    </div>}
  </div></Center>);
}
...
function CRM({config,user,supabase,plan,subscriptionEnd,themeId,onThemeChange,onUpgrade,onReset,onSignOut}){
  ...
  // ── Client sales history (ventas cerradas previas) ──────────
  const [clientSales, setClientSales] = useState([]);
  const [showNewSaleModal, setShowNewSaleModal] = useState(false);
  const [newSaleForm, setNewSaleForm] = useState({concept:"", value:"", notes:""});
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      if(alive) setClientSales(data||[]);
    })();
    return ()=>{ alive = false; };
  }, [user?.id]);

  // Helper: ventas previas de un cliente
  const salesForClient = (clientId) =>
    clientSales.filter(s => String(s.client_id) === String(clientId));

  // Helper: total facturado histórico (solo "won")
  const totalBilledForClient = (clientId) =>
    salesForClient(clientId)
      .filter(s => s.outcome === "won")
      .reduce((sum, s) => sum + Number(s.value || 0), 0);

  // Inicia nueva venta: archiva la actual en client_sales y resetea el cliente
  const startNewSale = async () => {
    if(!selected) return;
    const isWon = isWonStage(selected.stage);
    const isLost = isLostStage(selected.stage);
    if(!isWon && !isLost){
      showN("⚠ La venta actual no está cerrada");
      return;
    }
    const concept = newSaleForm.concept.trim() || `Proyecto cerrado en ${selected.stage}`;
    const valueNum = Number(newSaleForm.value || selected.value || 0);
    ...
    const salePayload = {
      ...
      stage_at_close: selected.stage,
      opened_at: selected.createdAt || null,
      closed_at: new Date().toISOString(),
      notes: newSaleForm.notes.trim() || null,
    };

    // 1) Insertar en client_sales
    const {data:inserted, error:insErr} = await sb
      .from("client_sales").insert(salePayload).select().single();
    if(insErr){ console.error(insErr); showN("⚠ Error al archivar venta"); return; }
    setClientSales([inserted, ...clientSales]);

    // 2) Actividad automática en el historial
    const act = {
      id: Date.now(),
      type: isWon ? "🔁 Nueva venta" : "🔁 Reactivación",
      note: `Proyecto archivado: "${concept}" (${isWon?"ganado":"perdido"} en "${selected.stage}", $${valueNum.toLocaleString("es-MX")}). Se inicia nuevo proceso de venta.`,
      date: now(),
    };

    // 3) Resetear cliente: stage → primera etapa, value → 0, expectedClose → null
    const updatedClient = {
      ...selected,
      stage: firstStage,
      value: 0,
      expectedClose: null,
      stageHistory: [...(selected.stageHistory||[]), {stage: firstStage, date: now()}],
      activities: [act, ...(selected.activities||[])],
    };

    // Optimistic UI
    setClients(clients.map(c => c.id===selected.id ? updatedClient : c));
    ...
    if(supabase){
      await trySupabase(
        () => supabase.from("clients").update({
          ...
          value: 0,
          expected_close: null,
          stage_history: updatedClient.stageHistory,
          activities: updatedClient.activities,
        }).eq("id", selected.id)
      );
    }

    showN(`✓ Nueva venta iniciada para ${selected.name}`);
    setShowNewSaleModal(false);
    setNewSaleForm({concept:"", value:"", notes:""});
  };

  const deleteSale = async (saleId) => {
    if(!window.confirm("¿Eliminar esta venta del historial? No se puede deshacer.")) return;
    const {error} = await sb.from("client_sales").delete().eq("id", saleId);
    if(error){ console.error(error); showN("⚠ Error al eliminar"); return; }
    setClientSales(clientSales.filter(s => s.id !== saleId));
    showN("✓ Venta eliminada del historial");
  };
...
`);
  ...
  return(<div style={{minHeight:"100vh",background:T.bg,fontFamily:"Jost,sans-serif",color:T.text,display:"flex",flexDirection:"column"}}>
        ...
        {STAGES.map(stage=>{
          ...
          return(<div key={stage} onDragOver={e=>{e.preventDefault();setDragOver(stage);}} onDrop={()=>{if(dragging){moveStage(dragging,stage);setDragging(null);setDragOver(null);}}} onDragLeave={()=>setDragOver(null)} style={{flex:"0 0 calc(85vw)",maxWidth:280,scrollSnapAlign:"start",background:dragOver===stage?T.cardHover:T.surface,borderRadius:2,border:`1px solid ${dragOver===stage?col+"80":T.border}`,display:"flex",flexDirection:"column",transition:"all .2s"}}>
            ...
            <div style={{flex:1,overflowY:"auto",padding:7}}>
              {scs.map(client=>{
                const wonCount = clientSales.filter(s=>String(s.client_id)===String(client.id) && s.outcome==="won").length;
                return (<div key={client.id} className="clientCard" draggable onDragStart={()=>setDragging(client.id)} onClick={()=>{setSelected(client);setDetailTab("info");}} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:2,padding:"11px",marginBottom:6}}>
                <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
                  <span style={{fontWeight:500,fontSize:13,flex:1,minWidth:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{client.name}</span>
                  {wonCount>0 && <span title={`${wonCount} venta(s) cerradas previamente`} style={{fontSize:9,color:T.green,background:`${T.green}15`,border:`1px solid ${T.green}40`,borderRadius:2,padding:"1px 5px",letterSpacing:.5,whiteSpace:"nowrap"}}>🔁 ×{wonCount}</span>}
                </div>
      ...
      <div style={{width:400,background:T.surface,borderLeft:`1px solid ${T.border}`,height:"100%",display:"flex",flexDirection:"column",animation:"slideR .25s"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"16px 20px"}}>
          ...
          {detailTab==="info"&&(<div>
            ...
            {/* HISTORIAL DE VENTAS · proyectos cerrados anteriormente */}
            {(()=>{
              const sales = salesForClient(selected.id);
              if(sales.length===0) return null;
              const wonSales = sales.filter(s=>s.outcome==="won");
              const totalBilled = wonSales.reduce((sum,s)=>sum+Number(s.value||0),0);
              return (
                <div style={{marginTop:18,padding:"14px 14px 10px",background:T.card,border:`1px solid ${T.border}`,borderRadius:2}}>
                  <SectionTitle>🔁 Historial de ventas</SectionTitle>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
                    <div style={{padding:"8px 10px",background:T.surface,borderRadius:2,border:`1px solid ${T.border}`}}>
                      <div style={{fontSize:9,color:T.textMuted,letterSpacing:1.5,textTransform:"uppercase",marginBottom:2}}>Proyectos cerrados</div>
                      <div style={{fontSize:18,color:T.gold,fontFamily:"Cormorant Garamond,serif",fontWeight:700}}>{wonSales.length}{sales.length>wonSales.length?<span style={{fontSize:11,color:T.textDim,marginLeft:4}}>+{sales.length-wonSales.length} perdidos</span>:""}</div>
                    </div>
                    <div style={{padding:"8px 10px",background:T.surface,borderRadius:2,border:`1px solid ${T.border}`}}>
                      <div style={{fontSize:9,color:T.textMuted,letterSpacing:1.5,textTransform:"uppercase",marginBottom:2}}>Total facturado</div>
                      <div style={{fontSize:18,color:T.green,fontFamily:"Cormorant Garamond,serif",fontWeight:700}}>{fmt(totalBilled)}</div>
                    </div>
                  </div>
                  <div style={{display:"flex",flexDirection:"column",gap:6}}>
                    {sales.map(s=>(
                      <div key={s.id} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",background:T.surface,borderRadius:2,border:`1px solid ${T.border}`}}>
                        <span style={{fontSize:14}}>{s.outcome==="won"?"✓":"✕"}</span>
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{fontSize:12,color:T.text,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.concept || "(sin concepto)"}</div>
                          <div style={{fontSize:10,color:T.textMuted}}>
                            {fmtDate(s.closed_at)} · {s.stage_at_close}
                            {s.notes ? ` · ${s.notes}` : ""}
                          </div>
                        </div>
                        <div style={{fontSize:12,color:s.outcome==="won"?T.green:T.textMuted,fontWeight:600,whiteSpace:"nowrap"}}>{fmt(Number(s.value||0))}</div>
                        <button onClick={()=>deleteSale(s.id)} title="Eliminar del historial"
                          style={{background:"transparent",border:"none",color:T.textDim,fontSize:14,cursor:"pointer",padding:"0 4px"}}>×</button>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {/* BOTÓN: Iniciar nueva venta (solo si la actual está cerrada) */}
            {(isWonStage(selected.stage) || isLostStage(selected.stage)) && (
              <div style={{marginTop:14}}>
                <button className="pBtn" onClick={()=>{
                  setNewSaleForm({concept:"", value:String(selected.value||0), notes:""});
                  setShowNewSaleModal(true);
                }}
                  style={{width:"100%",background:T.grad,color:T.gradText,padding:"11px",borderRadius:2,fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase",border:"none",cursor:"pointer"}}>
                  🔁 Iniciar nueva venta con este cliente
                </button>
                <div style={{fontSize:10,color:T.textDim,marginTop:6,textAlign:"center",lineHeight:1.4}}>
                  Archiva el proyecto actual en el historial y reinicia el pipeline conservando contacto, notas, archivos y actividades.
                </div>
              </div>
            )}
          ...
          {detailTab==="activity"&&(<div>
            ...
            <button className="pBtn" onClick={addActivity} style={{width:"100%",background:T.grad,color:T.gradText,padding:"9px",borderRadius:2,fontSize:10,fontWeight:700,letterSpacing:2,textTransform:"uppercase",marginBottom:18}}>◈ Registrar</button>
            <GL/>
            <div style={{marginTop:14}}>
          ...
          {detailTab==="tasks"&&(<div>
            ...
            <button className="pBtn" onClick={addTask} style={{width:"100%",background:T.grad,color:T.gradText,padding:"9px",borderRadius:2,fontSize:10,fontWeight:700,letterSpacing:2,textTransform:"uppercase",marginBottom:18}}>◈ Agregar tarea</button>
            <GL/>
            <div style={{marginTop:14}}>
              <Label>Tareas</Label>
    ...
    {showForm&&(<div style={{position:"fixed",inset:0,background:"rgba(44,36,32,.6)",zIndex:300,display:"flex",alignItems:"flex-start",justifyContent:"center",animation:"fadeIn .2s",padding:16,overflowY:"auto",overflowX:"hidden"}} onClick={()=>setShowForm(false)}>
      <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:2,width:"100%",maxWidth:540,maxHeight:"92vh",display:"flex",flexDirection:"column",overflowX:"hidden",animation:"fadeUp .25s"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{padding:"14px 24px",borderTop:`1px solid ${T.border}`,display:"flex",gap:10,flexShrink:0}}>
          ...
          <button className="pBtn" onClick={save} style={{flex:2,background:T.grad,color:T.gradText,padding:"10px",borderRadius:2,fontSize:10,fontWeight:700,letterSpacing:2,textTransform:"uppercase",boxShadow:`0 4px 20px ${T.gold}25`}}>
    ...
    {/* Modal: Iniciar nueva venta para cliente cerrado */}
    {showNewSaleModal && selected && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.55)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1100,padding:16}}
        ...
        <div onClick={e=>e.stopPropagation()} style={{background:T.bg,border:`1px solid ${T.border}`,borderRadius:2,padding:20,width:"100%",maxWidth:460,maxHeight:"90vh",overflow:"auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <SectionTitle>🔁 Nueva venta · {selected.name}</SectionTitle>
            <button onClick={()=>setShowNewSaleModal(false)} style={{background:"transparent",border:"none",color:T.textMuted,fontSize:20,cursor:"pointer"}}>×</button>
          </div>

          <div style={{fontSize:11,color:T.textMuted,marginBottom:14,lineHeight:1.5,padding:"10px 12px",background:T.card,borderRadius:2,border:`1px solid ${T.border}`}}>
            Vamos a archivar el proyecto actual <strong style={{color:T.text}}>"{selected.stage}"</strong> con valor <strong style={{color:T.text}}>{fmt(Number(selected.value||0))}</strong> en el historial de ventas, y reiniciar el pipeline en <strong style={{color:T.gold}}>"{STAGES[0]||"Lead"}"</strong> con valor 0.
          </div>

          <Label>Concepto del proyecto cerrado</Label>
          <input value={newSaleForm.concept} onChange={e=>setNewSaleForm({...newSaleForm,concept:e.target.value})}
            placeholder="Ej. Diseño de logo, Sitio web inicial..."
            style={{width:"100%",background:T.card,border:`1px solid ${T.border}`,borderRadius:2,padding:"10px 12px",color:T.text,fontSize:13,fontFamily:"Jost,sans-serif",marginBottom:10}}/>

          <Label>Valor cerrado (MXN)</Label>
          <input type="number" value={newSaleForm.value} onChange={e=>setNewSaleForm({...newSaleForm,value:e.target.value})}
            placeholder="0"
            style={{width:"100%",background:T.card,border:`1px solid ${T.border}`,borderRadius:2,padding:"10px 12px",color:T.text,fontSize:13,fontFamily:"Jost,sans-serif",marginBottom:10}}/>

          <Label>Notas (opcional)</Label>
          <textarea value={newSaleForm.notes} onChange={e=>setNewSaleForm({...newSaleForm,notes:e.target.value})} rows={3}
            placeholder="Detalles del proyecto, aprendizajes, próximos pasos..."
            style={{width:"100%",background:T.card,border:`1px solid ${T.border}`,borderRadius:2,padding:"10px 12px",color:T.text,fontSize:13,fontFamily:"Jost,sans-serif",lineHeight:1.5,resize:"vertical",marginBottom:14}}/>

          <div style={{display:"flex",gap:8}}>
            <button onClick={()=>setShowNewSaleModal(false)}
              style={{flex:1,background:"transparent",color:T.textMuted,padding:"11px",borderRadius:2,fontSize:11,border:`1px solid ${T.border}`,letterSpacing:1,cursor:"pointer"}}>
              Cancelar
            </button>
            <button onClick={startNewSale}
              style={{flex:2,background:T.grad,color:T.gradText,padding:"11px",borderRadius:2,fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase",border:"none",cursor:"pointer"}}>
              ◈ Archivar e iniciar nueva
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="fadeUp" style={{width:"100%",maxWidth:400}}>
        ...
        {!installed&&installPrompt&&(
          <div style={{marginTop:16,background:T.surface,border:`1px solid ${T.gold}50`,borderRadius:2,padding:"14px 16px",display:"flex",alignItems:"center",gap:12,animation:"fadeUp .4s ease"}}>
            ...
            <button className="pBtn" onClick={handleInstall} style={{background:T.grad,color:T.gradText,padding:"8px 14px",borderRadius:2,fontSize:11,fontWeight:700,letterSpacing:1,textTransform:"uppercase",flexShrink:0}}>
              Instalar
            </button>
          </div>
        )}
