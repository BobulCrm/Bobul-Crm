...
function CRM({config,user,supabase,plan,subscriptionEnd,themeId,onThemeChange,onUpgrade,onReset,onSignOut}){
  ...
  return(<div style={{minHeight:"100vh",background:T.bg,fontFamily:"Jost,sans-serif",color:T.text,display:"flex",flexDirection:"column"}}>
    ...
    <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,position:"sticky",top:0,zIndex:100}}>
      ...
      <div style={{padding:"0 12px",display:"flex",alignItems:"center",height:50,gap:8}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          <button className="pBtn" onClick={()=>{setShowSearch(s=>!s);if(showSearch)setSearch("");}} style={{background:showSearch?`${T.gold}15`:"transparent",border:`1px solid ${T.border}`,borderRadius:2,padding:"0 10px",fontSize:14,color:showSearch?T.gold:T.textMuted,flexShrink:0,height:32,display:"flex",alignItems:"center"}}>🔍</button>
      ...
      {showSearch&&(
        <div style={{padding:"0 12px 8px",animation:"fadeIn .15s"}}>
          <div style={{position:"relative"}}>
            ...
            <input autoFocus value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar cliente..."
              style={{...inputS,padding:"9px 10px 9px 28px",fontSize:13,width:"100%"}}/>
            {search&&<span onClick={()=>setSearch("")} style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",color:T.textMuted,fontSize:13,cursor:"pointer"}}>✕</span>}
          </div>
        </div>
      )}
    ...
    {/* -- STATS BAR */}
    <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"8px 6px",paddingBottom:view==="calendar"?2:8,display:"flex",alignItems:"stretch"}}>
      {[
    ...
    {/* -- CONTENT */}
    <div style={{flex:1,overflow:"auto",overflowX:"hidden",padding:18,paddingTop:view==="calendar"&&calView==="month"?0:18,maxWidth:"100vw"}}>
    ...
    {/* ── CALENDAR VIEW */}
    {view==="calendar"&&(
      <div style={{maxWidth:900,margin:"0 auto",marginTop:calView==="month"?0:-20,minHeight:"calc(100vh - 200px)"}}>
        {/* Calendar Header */}
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <button className="pBtn" onClick={()=>{const d=new Date(calDate);if(calView==="day")d.setDate(d.getDate()-1);else d.setMonth(d.getMonth()-1);setCalDate(d);}} style={{background:T.card,border:`1px solid ${T.border}`,color:T.text,padding:"6px 12px",borderRadius:2,fontSize:14}}>←</button>
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <button className="pBtn" onClick={()=>{const d=calView==="day"?`${calDate.getFullYear()}-${String(calDate.getMonth()+1).padStart(2,"0")}-${String(calDate.getDate()).padStart(2,"0")}`:today();setEditingEvent(null);setEventForm({title:"",date:d,time:"10:00",type:"Llamada",clientId:"",notes:""});setShowEventForm(true);}} style={{background:T.grad,color:"#fff",padding:"6px 14px",borderRadius:2,fontSize:11,fontWeight:700,letterSpacing:1,textTransform:"uppercase"}}>+ Evento</button>
          </div>
        </div>
        ...
        <div style={{display:"flex",justifyContent:"center",gap:0,marginBottom:6}}>
          {[["month","Mes"],["day","Día"]].map(([v,label])=>(
            <button key={v} className="pBtn" onClick={()=>setCalView(v)} style={{
              ...
              borderRadius:0,
              marginLeft:v==="day"?-1:0,
            }}>{label}</button>
          ))}
        </div>

        {/* Divider 2 — less bottom margin on month view to compensate for the weekday header row */}
        ...
        {calView==="day"&&(()=>{
          ...
          return(
            <div style={{position:"relative",background:T.surface,border:`1px solid ${T.border}`,borderRadius:2,overflow:"hidden"}}>
              {/* Time grid */}
              {slots.map((s,i)=>(
                <div key={i} onClick={()=>{setEditingEvent(null);setEventForm({title:"",date:dateStr,time:s.label,type:"Llamada",clientId:"",notes:""});setShowEventForm(true);}} style={{
              ...
              <div style={{position:"absolute",top:0,left:64,right:0,pointerEvents:"none"}}>
                {dayEvents.map(ev=>{
                  ...
                  return(
                    <div key={ev.id} onClick={e=>{e.stopPropagation();setEditingEvent(ev);setEventForm({...ev});setShowEventForm(true);}} style={{
                      position:"absolute",top:t+1,left:6,right:8,minHeight:slotH-4,
                      background:`${bg}1A`,borderLeft:`3px solid ${bg}`,borderRadius:2,
                      padding:"4px 8px",cursor:"pointer",pointerEvents:"auto",
                    }}>
                      <div style={{fontSize:11,fontWeight:600,color:T.text,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{ev.title}</div>
                      <div style={{fontSize:10,color:T.textMuted,marginTop:1}}>{ev.time} · {ev.type}{ev.clientId&&clients.find(c=>c.id==ev.clientId)?` · ${clients.find(c=>c.id==ev.clientId).name}`:""}</div>
                    </div>
                  );
                })}
              </div>
    ...
    <div style={{minHeight:"100vh",background:"#f5f2ee",display:"flex",alignItems:"center",justifyContent:"center",padding:24,fontFamily:"Jost,sans-serif"}}>
      <div style={{maxWidth:440,width:"100%"}}>
        ...
        <div style={{textAlign:"center",marginTop:20,fontSize:11,color:"#8a7968"}}>
          ¿Ya tienes cuenta?{" "}
          <span onClick={onSelectPlan} style={{color:"#b8935a",cursor:"pointer",textDecoration:"underline"}}>Volver al inicio</span>
        </div>
