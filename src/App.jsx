...
const T = {
  ...
  border:"#D8CFC4", borderGold:"#9A6F2050", text:"#1A1410", textMuted:"#5C4F42",
  textDim:"#A8998A", gold:"#7A5410", goldLight:"#9A6F20", goldDim:"#9A6F2018",
  accent:"#7A5410", grad:"linear-gradient(135deg,#9A6F20 0%,#5C3A08 100%)",
  ...
  return(<div style={{minHeight:"100vh",background:T.bg,fontFamily:"'Jost',sans-serif",color:T.text,display:"flex",flexDirection:"column"}}>
    ...
    <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"0 18px",display:"flex",alignItems:"center",height:54,gap:10,position:"sticky",top:0,zIndex:100}}>
      ...
      <div style={{display:"flex",border:`1px solid ${T.border}`,borderRadius:2,overflow:"hidden",marginLeft:"auto"}}>
        {[["kanban","Kanban"],["list","Lista"],["stats","Stats"],["tasks","Tareas"],["calendar","Agenda"]].map(([v,label])=>(
          <button key={v} className="pBtn" onClick={()=>setView(v)} style={{padding:"6px 12px",fontSize:10,letterSpacing:1,textTransform:"uppercase",background:view===v?T.gold+"20":"transparent",color:view===v?T.gold:T.textMuted,borderRight:v!=="calendar"?`1px solid ${T.border}`:"none"}}>{label}{v==="tasks"&&pendingTasks>0?` (${pendingTasks})`:""}</button>
        ))}
      </div>
...
<button className="pBtn" onClick={()=>setShowSettings(true)} style={{background:"transparent",color:T.textMuted,padding:"6px 9px",borderRadius:2,fontSize:11,border:`1px solid ${T.border}`,letterSpacing:.5}}>⚙ Config</button>
    ...
    {/* ── EVENT FORM MODAL */}
    {showEventForm&&(
      <div style={{position:"fixed",inset:0,background:"rgba(44,36,32,.6)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:16,animation:"fadeIn .2s"}} onClick={()=>setShowEventForm(false)}>
        <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:2,width:420,padding:24,animation:"fadeUp .25s"}} onClick={e=>e.stopPropagation()}>
    ...
    {showSettings&&(
      <div style={{position:"fixed",inset:0,background:"rgba(44,36,32,.6)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:16,animation:"fadeIn .2s"}} onClick={()=>setShowSettings(false)}>
        <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:2,width:520,maxHeight:"88vh",display:"flex",flexDirection:"column",animation:"fadeUp .25s"}} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{display:"flex",borderBottom:`1px solid ${T.border}`,flexShrink:0}}>
            {[["perfil","👤 Perfil"],["etapas","📋 Etapas"],["categoria","🏷️ Categoría"]].map(([id,label])=>(
          ...
          <div style={{flex:1,overflowY:"auto",padding:"18px 22px"}}>
            ...
            {settTab==="etapas"&&(
              <div>
                ...
                <button className="pBtn" onClick={async()=>{
                  if(supabase&&user) await supabase.from("profiles").update({stages:settStages}).eq("id",user.id);
                  showN("✅ Etapas actualizadas — recarga para ver cambios");
                  setShowSettings(false);
                }} style={{width:"100%",background:T.grad,color:"#fff",padding:"11px",borderRadius:2,fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase"}}>
                  Guardar etapas
                </button>
