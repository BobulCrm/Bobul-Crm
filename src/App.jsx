...
function CRM({config,user,supabase,plan,subscriptionEnd,themeId,onThemeChange,onUpgrade,onReset,onSignOut}){
  ...
  const saveEventNow = async (toSave) => {
    ...
    if(editingEvent){
      ...
      const newEvent={...toSave,id:String(Date.now())};
      // Optimistic add
      ...
      if(supabase&&user){
        ...
        if(res.ok && res.data && res.data[0]){
          setEvents(evs=>evs.map(e=>e.id===newEvent.id?{...e,id:String(res.data[0].id)}:e));
        }
      }
    }
  };
  ...
  return(<div style={{minHeight:"100vh",background:T.bg,fontFamily:"Jost,sans-serif",color:T.text,display:"flex",flexDirection:"column"}}>
    ...
    {/* ── CALENDAR VIEW */}
    {view==="calendar"&&(
      <div style={{maxWidth:1000,width:"100%",margin:"0 auto",marginTop:-20,minHeight:"calc(100vh - 200px)",overflow:"hidden",boxSizing:"border-box"}}>
