...
function CRM({config,user,supabase,plan,subscriptionEnd,themeId,onThemeChange,onUpgrade,onReset,onSignOut}){
  ...
  // Close category tooltip on outside click
  useEffect(()=>{
    if(!showCategoryTip) return;
    const close = ()=>setShowCategoryTip(false);
  ...
  return(<div style={{minHeight:"100vh",background:T.bg,fontFamily:"Jost,sans-serif",color:T.text,display:"flex",flexDirection:"column"}}>
    ...
    <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,position:"sticky",top:0,zIndex:100}}>
      ...
      <div style={{padding:"0 12px",display:"flex",alignItems:"center",height:50,gap:8}}>
        <div style={{display:"flex",alignItems:"center",gap:7,flexShrink:0}}>
          ...
          <div style={{cursor:"pointer"}} onClick={()=>setShowCategoryTip(true)}>
            <div style={{fontFamily:"Cormorant Garamond,serif",fontSize:14,fontWeight:600,color:T.text,lineHeight:1,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:110}}>{company||name}</div>
            <div style={{fontSize:8,color:T.textMuted,letterSpacing:1,textTransform:"uppercase",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:110}}>{profession?.label?.slice(0,22)}</div>
          </div>
        </div>
    ...
    {showSubscription&&(
      <div style={{position:"fixed",inset:0,background:"rgba(44,36,32,.6)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:16,animation:"fadeIn .2s"}} onClick={()=>setShowSubscription(false)}>
        ...
        {showCategoryTip&&(
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.35)",zIndex:600,display:"flex",alignItems:"flex-start",justifyContent:"flex-start",padding:"62px 16px 16px"}} onClick={()=>setShowCategoryTip(false)}>
        <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:2,padding:"16px 20px",minWidth:220,boxShadow:"0 8px 32px rgba(0,0,0,.2)",animation:"fadeUp .15s"}} onClick={e=>e.stopPropagation()}>
          <div style={{fontSize:9,color:T.gold,letterSpacing:1.5,textTransform:"uppercase",marginBottom:4}}>Negocio</div>
          <div style={{fontSize:15,fontWeight:600,color:T.text,fontFamily:"Cormorant Garamond,serif",marginBottom:12}}>{company||name}</div>
          <div style={{fontSize:9,color:T.gold,letterSpacing:1.5,textTransform:"uppercase",marginBottom:4}}>Categoría</div>
