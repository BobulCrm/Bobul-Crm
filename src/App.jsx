...
const I18N = {
  es: {
    ...
    privacyConsent:"Consentimiento de privacidad", privacyText:"El cliente ha dado su consentimiento para que sus datos sean tratados con fines comerciales, conforme al Aviso de Privacidad.",
    ...
    // Activity
    quickSend:"Enviar mensaje rapido", logActivity:"Registrar actividad manual", activityType:"Tipo de actividad",
    activityNote:"Nota / Descripción", activityDate:"Fecha", addActivity:"Agregar actividad",
        ...
        <div style={{flex:1,overflowY:"auto",overflowX:"hidden",padding:"18px 16px",width:"100%"}}>
          ...
          <div style={{marginBottom:14,borderBottom:`1px solid ${T.border}`,paddingBottom:14}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,width:"100%",minWidth:0}}>
              ...
              <div><Label>Fuente del lead</Label><select value={form.source} onChange={e=>setForm({...form,source:e.target.value})} style={inputS}><option value="">Seleccionar...</option>{LEAD_SOURCES_L.map(s=><option key={s}>{s}</option>)}</select></div>
      ...
      <div style={{position:"fixed",inset:0,background:"rgba(44,36,32,.6)",zIndex:300,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:16,paddingTop:40,overflowY:"auto",overflowX:"hidden",animation:"fadeIn .2s"}} onClick={()=>setShowEventForm(false)}>
        <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:2,width:"100%",maxWidth:420,padding:20,animation:"fadeUp .25s",overflowX:"hidden"}} onClick={e=>e.stopPropagation()}>
          ...
          {[
            ...
            {label:"Fecha",key:"date",type:"date",ph:""},
            {label:"Hora",key:"time",type:"time",ph:""},
          ].map(f=>(
