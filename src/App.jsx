            ...
            <div style={{position:"relative",background:T.surface,border:`1px solid ${T.border}`,borderRadius:2,overflow:"hidden"}}>
              ...
              <div style={{position:"absolute",top:0,left:64,right:0,pointerEvents:"none"}}>
                {dayEvents.map(ev=>{
                  ...
                  // Height is proportional to duration. Each 30-min slot = slotH px.
                  const rawH=(dur/30)*slotH-4;
                  // Clip the block so it doesn't overflow past 20:00
                  ...
                  return(
                    <div key={ev.id} onClick={e=>{e.stopPropagation();setEditingEvent(ev);setEventForm({...ev,duration:dur});setShowEventForm(true);}} style={{
                      position:"absolute",top:t+1,left:6,right:8,height:h,
                      ...
                      <div style={{fontSize:10,color:T.textMuted,marginTop:1,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{ev.time}–{endLabel} · {ev.type}{ev.clientId&&clients.find(c=>c.id==ev.clientId)?` · ${clients.find(c=>c.id==ev.clientId).name}`:""}</div>
                    </div>
                  );
                })}
              </div>
          ...
          <div style={{marginBottom:12}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:4,background:T.card,border:`1px solid ${[15,30,45,60,90,120,180,240].includes(Number(eventForm.duration))?T.border:T.gold}`,borderRadius:2,padding:"3px 8px"}}>
                <input type="number" min="5" max="720" step="5" value={eventForm.duration||60} onChange={e=>setEventForm(ef=>({...ef,duration:Number(e.target.value)||60}))}
