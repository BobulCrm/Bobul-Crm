...
function CRM({config,user,supabase,plan,onUpgrade,onReset,onSignOut}){
  ...
  return(<div style={{minHeight:"100vh",background:T.bg,fontFamily:"'Jost',sans-serif",color:T.text,display:"flex",flexDirection:"column"}}>
    ...
    <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"0 18px",display:"flex",alignItems:"center",height:54,gap:10,position:"sticky",top:0,zIndex:100}}>
      ...
      <select value={view} onChange={e=>setView(e.target.value)} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:2,padding:"6px 10px",color:T.gold,fontSize:11,fontFamily:"'Jost',sans-serif",fontWeight:600,marginLeft:"auto",maxWidth:120}}>
        {[["kanban","⬛ Kanban"],["list","≡ Lista"],["stats","◎ Stats"],["tasks",`✓ Tareas${pendingTasks>0?" ("+pendingTasks+")":""}`],["calendar","📅 Agenda"]].map(([v,label])=>(
          <option key={v} value={v}>{label}</option>
        ))}
      </select>

      <button className="pBtn" onClick={()=>{setShowForm(true);setEditing(null);resetForm();}} style={{background:T.grad,color:"#fff",padding:"7px 14px",borderRadius:2,fontSize:11,fontWeight:700,letterSpacing:1,textTransform:"uppercase",flexShrink:0}}>+ Nuevo</button>
      <div style={{position:"relative",flexShrink:0}}>
        <button className="pBtn" onClick={()=>setShowMobileMenu(v=>!v)} style={{background:T.card,border:`1px solid ${T.border}`,color:T.textMuted,padding:"6px 11px",borderRadius:2,fontSize:18}}>⋮</button>
        {showMobileMenu&&(<div style={{position:"absolute",top:"110%",right:0,background:T.surface,border:`1px solid ${T.border}`,borderRadius:2,boxShadow:"0 8px 24px rgba(0,0,0,.12)",zIndex:200,minWidth:170,padding:6}}>
